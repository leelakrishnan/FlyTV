...
const TeamMembers = ({ teamData }: Props) => {
  ...
  function loadTeamInfo() {
    async function getMemberInfo(membersIds: string[]) {
      ...
      userQuery.containedIn("objectId", membersIds);
      ...
      if (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          let firstName = data[i]?.get("firstName")
            ? data[i]?.get("firstName")
            : "";
          let lastName = data[i]?.get("lastName")
            ? data[i]?.get("lastName")
            : "";
          let emailAddress = data[i]?.get("emailAddress")
            ? data[i]?.get("emailAddress")
            : "";
          let githubUserName = data[i]?.get("githubUserName")
            ? data[i]?.get("githubUserName")
            : "";
    ...
    if (userInfo && userInfo.length > 0) {
      if (userInfo && userInfo[0] && userInfo[0].id) {
        ...
        if (teamData && teamData.length > 0) {
          ...
          let firstName = teamData[0]?.get("firstName")
            ? teamData[0]?.get("firstName")
            : "";
          let lastName = teamData[0]?.get("lastName")
            ? teamData[0]?.get("lastName")
            : "";
          let emailAddress = teamData[0]?.get("emailAddress")
            ? teamData[0]?.get("emailAddress")
            : "";
          let githubUserName = teamData[0]?.get("githubUserName")
            ? teamData[0]?.get("githubUserName")
            : "";
